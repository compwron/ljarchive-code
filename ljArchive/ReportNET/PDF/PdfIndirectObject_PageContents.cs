...
namespace Root.Reports {
  ...
  internal class PdfIndirectObject_PageContents : PdfIndirectObject, IPdfRepObjX {
    ...
    internal void Write_Font(FontProp fontProp) {
      ...
      Type1FontData type1FontData = (Type1FontData)fontProp.fontPropData.fontData;
      PdfFontPropData pdfFontPropData = (PdfFontPropData)fontProp.fontPropData;
      String sFont_New = pdfFontPropData.pdfIndirectObject_Font.iObjectNumber + " " + RT.sPdfDim(type1FontData.rSizeFactor(fontProp));
      if (sFont_Cur != sFont_New) {
        Name("F" + pdfFontPropData.pdfIndirectObject_Font.iObjectNumber);  Space();  Number(type1FontData.rSizeFactor(fontProp));  Command("Tf");
        sFont_Cur = sFont_New;
      }
    }
