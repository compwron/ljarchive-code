...
namespace Root.Reports {
  ...
  /// </remarks>
  internal class TrueTypeFontData : FontData {
    //------------------------------------------------------------------------------------------16.02.2005
    ...
    internal TrueTypeFontData(FontDef fontDef, FontDef.Style style) : base(style, FontData.Encoding.Cp1252) {
      ...
      sFontFileName = Path.Combine(sSystemRoot, sFontFileName);

      StreamReader streamReader = new StreamReader(sFontName);
      try {
...
//      return rSizeFactor(fontProp) * fontData.rRawWidth(sText) / 1000;
    ...
    internal class CharMetrics {
      ...
      internal CharMetrics(TrueTypeFontData type1FontData, String sLine) {
        #if WindowsCE
